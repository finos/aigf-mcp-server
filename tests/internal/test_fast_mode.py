...
try:
    from finos_mcp.internal.mock_content_service import (
        get_mock_content_service as get_content_service,
    )
    from finos_mcp.internal.testing import (
        FastTestMode,
        InMemoryCache,
        InMemoryHTTPClient,
    )
...
class TestFastTestMode:
    ...
    async def test_fast_mode_70_percent_speedup(self):
        ...
        with patch("finos_mcp.content.service.get_content_service") as mock_service:
            ...
            mock_service.return_value.get_document.side_effect = (
                lambda *args: asyncio.sleep(0.1)
            )
...
class TestFastModeIntegration:
    ...
    async def test_fast_mode_with_content_service(self):
        ...
        with FastTestMode() as fast_mode:
            ...
            mitigation = await service.get_document(
                "mitigation", "sample-mitigation.md"
            )
