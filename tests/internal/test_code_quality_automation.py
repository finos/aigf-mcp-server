...
class TestQualityGate:
    ...
    def test_gate_creation(self):
        ...
        gate = QualityGate(min_coverage=80.0, require_tests=True, max_complexity=10)
    ...
    async def test_full_quality_check(self):
        ...
        with (
            patch.object(gate, "_check_linting") as mock_lint,
...
class TestPreCommitHook:
    ...
    def test_hook_creation(self):
        ...
        hook = PreCommitHook(auto_fix=True, run_tests=True, check_coverage=True)
    ...
    async def test_get_staged_files(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(
                returncode=0, stdout="src/module.py\ntests/test_module.py\n"
            )
    ...
    async def test_focused_test_running(self):
        ...
        # Mock test discovery
        with (
            patch.object(hook, "_find_related_tests") as mock_find,
    ...
    async def test_full_pre_commit_flow(self):
        ...
        with (
            patch.object(hook, "_get_staged_files") as mock_files,
    ...
    async def test_quality_gate_with_auto_fixes(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            mock_run.side_effect = [
                Mock(
                    returncode=1, stderr="Formatting issues found"
                ),  # Initial linting fails
            ...
            with patch.object(hook, "_get_staged_files") as mock_staged:
                ...
                if missing_tests:
                    ...
                    test_code = generator.generate_test_file(
                        {"name": "module", "functions": ["func"], "classes": []}
                    )
                    test_file.write_text(test_code)

                # Step 3: Quality gate validation
                with patch("subprocess.run") as mock_run:
                    # Mock coverage output with proper format
                    coverage_output = "TOTAL                      100     0   100%"
                    mock_run.return_value = Mock(returncode=0, stdout=coverage_output)
